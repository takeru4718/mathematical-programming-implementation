...
using Adaptive = eax::EAX_normal<AssemblerBuilder, eax::AdaptiveRangeMerger, eax::TabuABCycleFinder>;
    ...
    auto crossover_func = [&adaptive_n_ab, &adaptive_rand, &adaptive_uniform](const Individual& parent1, const Individual& parent2,
        ...
        struct {
            ...
            auto operator()(const eax::EAX_Rand_tag&) {
                return eax_rand(parent1, parent2, context.env.num_children, context.env.tsp, context.random_gen, {}, std::forward_as_tuple(context.pop_edge_counts, context.env.range_size), std::forward_as_tuple(*tabu_edges_ptr));
            }

            auto operator()(const eax::EAX_n_AB_tag& n_ab) {
                return eax_n_ab(parent1, parent2, context.env.num_children, context.env.tsp, context.random_gen, n_ab.get_n(), std::forward_as_tuple(context.pop_edge_counts, context.env.range_size), std::forward_as_tuple(*tabu_edges_ptr));
            }

            auto operator()(const eax::EAX_UNIFORM_tag& uniform) {
                return eax_uniform(parent1, parent2, context.env.num_children, context.env.tsp, context.random_gen, uniform.get_ratio(), std::forward_as_tuple(context.pop_edge_counts, context.env.range_size), std::forward_as_tuple(*tabu_edges_ptr));
            }
        } visitor {adaptive_n_ab, adaptive_rand, adaptive_uniform, parent1, parent2, context, tabu_edges_ptr};
