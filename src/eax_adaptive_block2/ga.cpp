...
std::pair<mpi::genetic_algorithm::TerminationReason, std::vector<Individual>> execute_ga(
    ...
    eax::EAX_normal<eax::N_AB_e_set_assembler_builder, eax::AdaptiveRangeMerger> adaptive_n_ab(object_pools);
    eax::EAX_Block2 eax_block2(object_pools);
    auto crossover_func = [&adaptive_n_ab, &eax_block2](const Individual& parent1, const Individual& parent2,
                                Context& context) {
        auto& env = context.env;
        switch (context.eax_type) {
            case eax::EAXType::One_AB:
                return adaptive_n_ab(parent1, parent2, env.num_children, env.tsp, context.random_gen, 1, std::forward_as_tuple(context.pop_edge_counts, env.range_size));
