...
void execute_normal(const Arguments& args)
{
    ...
    for (size_t trial = 0; trial < args.trials; ++trial) {
        ...
        // 環境
        eax::Environment ga_env{tsp, args.population_size, args.num_children, selection_type, local_seed, eax_type, args.num_reference_parents};
...
int main(int argc, char* argv[])
{
    ...
    eax_type_spec.set_description("--eax-type <type> \t:EAX crossover type. Options are 'EAX_1_AB' (default), 'EAX_Rand', and 'EAX_UNIFORM'. EAX_{N}_AB is also supported, where {N} is a positive integer.");
    parser.add_argument(eax_type_spec);

    mpi::ArgumentSpec num_reference_parents_spec(args.num_reference_parents);
    num_reference_parents_spec.add_argument_name("--num-reference-parents");
    num_reference_parents_spec.set_description("--num-reference-parents <number> \t:Number of reference parents to use in crossover (default: 10).");
    parser.add_argument(num_reference_parents_spec);
