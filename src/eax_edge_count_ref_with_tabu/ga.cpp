...
using ECRef = eax::EAX_normal<AssemblerBuilder, eax::EdgeCountReferenceMerger, eax::TabuABCycleFinder>;
}
    ...
    auto crossover_func = [&eax_n_ab, &eax_rand, &eax_uniform](const Individual& parent1, const Individual& parent2,
        ...
        struct {
            ...
            auto operator()(const eax::EAX_Rand_tag&) {
                return eax_rand(parent1, parent2, context.env.num_children, context.env.tsp, context.random_gen, {}, context.edge_counter, std::forward_as_tuple(*tabu_edges_ptr));
            }

            auto operator()(const eax::EAX_n_AB_tag& n_ab) {
                return eax_n_ab(parent1, parent2, context.env.num_children, context.env.tsp, context.random_gen, n_ab.get_n(), context.edge_counter, std::forward_as_tuple(*tabu_edges_ptr));
            }

            auto operator()(const eax::EAX_UNIFORM_tag& uniform) {
                return eax_uniform(parent1, parent2, context.env.num_children, context.env.tsp, context.random_gen, uniform.get_ratio(), context.edge_counter, std::forward_as_tuple(*tabu_edges_ptr));
            }
        } visitor {eax_n_ab, eax_rand, eax_uniform, parent1, parent2, context, tabu_edges_ptr};
