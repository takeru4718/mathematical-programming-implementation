...
namespace eax {
class EdgeCountReferenceMerger {
public:
    EdgeCountReferenceMerger(ObjectPools& object_pools)
    ...
    void operator()(IntermediateIndividual& working_individual,
        ...
        while (subtour_list.sub_tour_count() > 1) {
            ...
            for (size_t i = 1; i <= min_sub_tour_size; ++i) {
                ...
                auto& existing_edges_connecting_current_city = edge_counter.get_connected_vertices(current_city);
                size_t limit = existing_edges_connecting_current_city.size();
                ...
                for (size_t j = 0; j < limit; ++j) {
                    ...
                    for (size_t k = 0; k < 2; ++k) {
                        ...
                        for (size_t l = 0; l < 2; ++l) {
                            size_t connected_to_candidate_city = working_individual[candidate_city][l];

                            distance_type cost = - adjacency_matrix[current_city][connected_to_current_city] - adjacency_matrix[candidate_city][connected_to_candidate_city]
                                                + adjacency_matrix[current_city][candidate_city] + adjacency_matrix[connected_to_current_city][connected_to_candidate_city];
                            
                            if (cost < min_cost) {
                                min_cost = cost;
                                e1 = {current_city, connected_to_current_city};
                                e2 = {candidate_city, connected_to_candidate_city};
                            }
                        }
                    }
                }
            ...
            // 必ず見つかる
            assert((e1 != edge{0, 0} && e2 != edge{0, 0}));
