...
void execute_normal(const Arguments& args)
{
    ...
    for (size_t trial = 0; trial < args.trials; ++trial) {
        ...
        if (termination_reason == mpi::genetic_algorithm::TerminationReason::TimeLimit) {
            cout << "Genetic algorithm terminated due to time limit." << endl;
        } else {
            // 結果を出力
...
int main(int argc, char* argv[])
{
    ...
    eax_type_spec.set_description("--eax-type <type> \t:EAX crossover type. Options are 'EAX_5_AB' (default), 'EAX_Rand', and 'EAX_UNIFORM'. EAX_{N}_AB is also supported, where {N} is a positive integer.");
